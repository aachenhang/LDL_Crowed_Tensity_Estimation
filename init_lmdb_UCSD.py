...
parser.add_argument('--data', type=str, help='Morph database directory', required=False, default='frames_UCSD/')
...
if __name__ == '__main__':
    ...
    base_dir = abspath(join(args.data, '..', 'cc', 'data', 'UCSDDB'))


    #Shuffle the train set
    train_list = copy.deepcopy(img_list[600:1400])
    ...
    #make_lmdb(db_path, img_list[:NTrain], 'count')
    make_lmdb(db_path, train_list, 'count')

    #get the test set without shuffling
    test_list = copy.deepcopy(img_list[0:600]+img_list[1400:2000])
    ...
    with open(abspath(join(base_dir, 'Ratio' + str(args.ratio), 'db.info')), 'w') as db_info:
        ...
        if not isdir("data"):
            os.makedirs("data")
        if not isdir("data/UCSDDB"):
            os.symlink(base_dir, "data/UCSDDB")
            print "Make data symbol link at 'data/UCSDDB'."
